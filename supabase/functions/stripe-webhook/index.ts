...
const getProPriceId = () => Deno.env.get("STRIPE_PRICE_ID_PRO") || Deno.env.get("VITE_STRIPE_PRICE_ID_PRO");

const isPayingSubscriptionStatus = (status?: string | null) => status === "active" || status === "trialing";

const resolveTierForPaidPrice = (priceId?: string | null, status?: string | null) => {
...
const requireUpdatedRows = (data: any[] | null, context: string) => {
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        const tier = resolveTierForPaidPrice(priceId, subscriptionStatus);
        console.log(`[Stripe Webhook] Tier determined: ${tier} (price ${priceId}, status ${subscriptionStatus})`);
        ...
        requireUpdatedRows(data, `Profile update for ${userId}`);
      ...
      case "invoice.payment_succeeded": {
        ...
        if (subscriptionId) {
          ...
          const tier = resolveTierForPaidPrice(priceId, subscription.status);
          ...
          requireUpdatedRows(data, `Profile update for customer ${customerId}`);
      ...
      case "customer.subscription.created": {
        ...
        requireUpdatedRows(data, `Subscription update for customer ${customerId}`);
      ...
      case "customer.subscription.deleted": {
        ...
        requireUpdatedRows(data, `Subscription delete for customer ${customerId}`);
        break;
      }
    }
