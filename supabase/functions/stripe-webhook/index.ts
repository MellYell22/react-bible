...
const proSubscriptionStatuses = new Set(["active", "trialing"]);

const tierForSubscription = (priceId: string | undefined, status: string | undefined) => {
  const proPriceId = Deno.env.get("STRIPE_PRICE_ID_PRO");
  if (!proPriceId) {
    throw new Error("STRIPE_PRICE_ID_PRO is not set");
  }

  return priceId === proPriceId && status && proSubscriptionStatuses.has(status)
    ? "pro"
    : "free";
};
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        const tier = tierForSubscription(priceId, subscriptionStatus);
        console.log(`[Stripe Webhook] Price ID from session: ${priceId}, status: ${subscriptionStatus}, tier: ${tier}`);
        ...
        if (!data || data.length === 0) {
          throw new Error(`Profile update for ${userId} affected 0 rows`);
        } else {
      ...
      case "invoice.payment_succeeded": {
        ...
        if (subscriptionId) {
          ...
          const tier = tierForSubscription(priceId, subscription.status);

          console.log(`[Stripe Webhook] Updating customer ${customerId} to tier: ${tier}, status: ${subscription.status}`);
          ...
          if (!data || data.length === 0) {
            throw new Error(`Profile update for customer ${customerId} affected 0 rows`);
          } else {
      ...
      case "customer.subscription.created": {
        ...
        if (!data || data.length === 0) {
          throw new Error(`Subscription update for customer ${customerId} affected 0 rows`);
        } else {
    ...
    // Return a non-2xx response so Stripe retries failed entitlement writes.
